...
export function generatePassport(input) {
    ...
    const passportWithoutSignature = {
        schemaVersion: "0.1",
...
function buildRecommendations(risk, sensitiveCount, dependencyChanges, testsChanged, workflowChanges) {
    const recommendations = ["Review the generated passport before merge and confirm the changed scope matches the PR intent."];
    if (sensitiveCount > 0)
        recommendations.push("Request focused human review for every sensitive file listed in the passport.");
    if (dependencyChanges)
        recommendations.push("Review dependency names, lockfile changes, licenses, and vulnerability impact.");
    if (!testsChanged)
        recommendations.push("Confirm existing tests cover the changed behavior or ask for targeted tests.");
    if (workflowChanges)
        recommendations.push("Review workflow permissions, secrets usage, third-party actions, and deployment triggers.");
    if (risk === "critical" || risk === "high")
        recommendations.push("Require explicit maintainer approval before merging.");
