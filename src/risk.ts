...
export function scoreChange(files: ChangedFile[], touches: SensitiveTouch[], dependencyChanges: boolean, testsChanged: boolean): { score: number; risk: RiskLevel } {
...
function toRisk(score: number, hasCriticalTouch: boolean): RiskLevel {
  if (hasCriticalTouch || score >= 75) return "critical";
